...
for idx, s in enumerate(sec_splits):
    ...
    if sec_tag in ['PRIVILEGE', 'DEDICATION-SIGISMUND', 'HERB-VERSE', 'PREFACE-SAPIEHA']:
        ...
        # It is one of chapters CH-01 to CH-14
        # Articles in 1588 are denoted by 'АРТЫКУЛЪ X.' or 'АРТЫКУЛ X.' or 'АРТЫКУЛЪ'
        # Let's inspect article matches
        art_matches = list(re.finditer(r'(?:^|\n)\s*АРТЫКУЛ[Ъ]?\s+(\d+)\.?', sec_text))
        if not art_matches:
            units_1588.append({
                'section_tag': sec_tag,
                'chapter': sec_title,
            ...
            preface = sec_text[:art_matches[0].start()].strip()
            if len(preface) > 50:
                units_1588.append({
                    'section_tag': sec_tag,
                    'chapter': sec_title,
            ...
            for j in range(len(art_matches)):
                a_num = art_matches[j].group(1)
                a_start = art_matches[j].start()
                a_end = art_matches[j+1].start() if j+1 < len(art_matches) else len(sec_text)
...
print(f"Total document/article units parsed in LS-1588: {len(units_1588)}")
